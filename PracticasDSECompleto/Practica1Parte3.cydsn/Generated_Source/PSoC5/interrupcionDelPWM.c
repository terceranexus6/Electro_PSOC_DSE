...
void interrupcionDelPWM_Stop(void)
{
    ...
    interrupcionDelPWM_SetVector(&IntDefaultHandler);
}
...
void interrupcionDelPWM_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)interrupcionDelPWM__INTC_NUMBER] = address;
}
...
cyisraddress interrupcionDelPWM_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)interrupcionDelPWM__INTC_NUMBER];
}
...
uint8 interrupcionDelPWM_GetState(void)
{
    ...
    return ((*interrupcionDelPWM_INTC_SET_EN & (uint32)interrupcionDelPWM__INTC_MASK) != 0u) ? 1u:0u;
}
